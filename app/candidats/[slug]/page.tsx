...
export async function generateMetadata({
  ...
  return {
    title: `${candidate.name} - ${candidate.globalScore}/10 | Gambella Policy Lab`,
...
function MeasureTag({ type }: { type: string }) {
  ...
  return (
    <span
      className={`text-[9px] uppercase tracking-[0.12em] px-1.5 py-0.5 rounded font-bold ${tag.bg} ${tag.text}`}
    >
      {tag.label}
    </span>
  )
  ...
  return (
    <div className="site-shell min-h-screen">
          ...
          <div className="hero-panel p-4 sm:p-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
              <div
                ...
                style={{
                  background: `linear-gradient(140deg, ${candidate.politicalColor}, ${candidate.politicalColor}77)`,
                }}
              >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-xs text-ink-3 font-semibold uppercase tracking-[0.12em]">
                    {candidate.party}
                  </span>
                </div>
                <h1 className="text-2xl sm:text-4xl font-bold text-ink tracking-tight">
                  {candidate.name}
                </h1>
              ...
              <div className="flex flex-col items-center gap-2 sm:gap-3 self-start sm:self-center">
                <div
                  className={`border rounded-xl px-4 py-3 sm:px-5 sm:py-4 ${getGlobalScoreBg(
                    candidate.globalScore
                  )}`}
                >
                  <div
                    className={`text-3xl sm:text-5xl font-bold text-center ${getScoreColor(
                      candidate.globalScore
                    )}`}
                  >
                    {candidate.globalScore}
                  </div>
                  <div className="text-xs text-ink-3 mt-0.5 text-center">/10 overall</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-5 sm:px-8 py-8 sm:py-12">
        <section
          className={`panel-card p-5 sm:p-6 mb-8 sm:mb-10 ${getGlobalScoreBg(
            candidate.globalScore
          )}`}
        >
          <div className="text-xs uppercase tracking-[0.12em] text-ink-3 font-semibold mb-2">
            Platform reading
          </div>
        ...
        <section className="panel-card p-6 sm:p-8 mb-8 sm:mb-10">
          <h2 className="text-xl sm:text-2xl font-bold text-ink mb-6">Criteria breakdown</h2>
          <ScoreRadar scores={candidate.scores} />
        </section>

        {candidate.thematicScores.length > 0 && (
          <section className="panel-card p-6 sm:p-8 mb-8 sm:mb-10">
            <h2 className="text-xl sm:text-2xl font-bold text-ink mb-6">Thematic scores</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
              {candidate.thematicScores.map((themeScore) => (
                <div key={themeScore.theme} className="playful-dash bg-white p-2.5 sm:p-4 text-center">
                  ...
                  <div className={`text-2xl font-bold ${getScoreColor(themeScore.score)}`}>
                    {themeScore.score.toFixed(1)}
                  </div>
                  <div className="mt-2 score-bar">
                    <div
                      className="score-bar-fill"
                      style={{
                        width: `${(themeScore.score / 10) * 100}%`,
                        backgroundColor: getBarColor(themeScore.score),
                      }}
                    />
                  </div>
                </div>
        ...
        <section className="grid md:grid-cols-2 gap-4 sm:gap-6 mb-8 sm:mb-10">
          <div className="panel-card p-6 sm:p-7">
            <h2 className="text-lg sm:text-xl font-bold text-ink mb-1">Strong measures</h2>
            <p className="text-xs text-ink-3 mb-5">
              Measures that appear coherent, practical, and comparatively robust.
            </p>
          ...
          <div className="panel-card p-6 sm:p-7">
            <h2 className="text-lg sm:text-xl font-bold text-ink mb-1">Weak measures</h2>
            <p className="text-xs text-ink-3 mb-5">
              Measures that appear unrealistic, weakly supported, or harder to implement.
            </p>
            <div className="space-y-3">
        ...
        <section className="grid md:grid-cols-2 gap-4 sm:gap-6 mb-8 sm:mb-10">
          <div className="panel-card p-6 sm:p-7">
            <h2 className="text-lg sm:text-xl font-bold text-ink mb-4">Strengths</h2>
            <ul className="space-y-3">
              {candidate.strengths.map((strength, index) => (
                <li
                  key={index}
                  className="flex items-start gap-3 text-sm text-ink-2 leading-relaxed"
                >
          ...
          <div className="panel-card p-6 sm:p-7">
            <h2 className="text-lg sm:text-xl font-bold text-ink mb-4">Weaknesses</h2>
            <ul className="space-y-3">
        ...
        {candidate.contradictions.length > 0 && (
          <section className="panel-card p-6 sm:p-7 mb-8 sm:mb-10">
            <h2 className="text-lg sm:text-xl font-bold text-ink mb-4">Internal contradictions</h2>
            <div className="space-y-3">
        ...
        {candidate.blindSpots.length > 0 && (
          <section className="panel-card p-6 sm:p-7 mb-8 sm:mb-10">
            <h2 className="text-lg sm:text-xl font-bold text-ink mb-4">Blind spots</h2>
       ...
       {(candidate.problematicMeasures ?? []).length > 0 && (
  <section className="panel-card p-6 sm:p-7 mb-8 sm:mb-10">
    <h2 className="text-lg sm:text-xl font-bold text-ink mb-4">Measures to clarify</h2>
    <div className="space-y-3">
      {(candidate.problematicMeasures ?? []).map((measure, index) => (
        ...
        <section
          className={`panel-card p-6 sm:p-8 mb-8 sm:mb-10 ${getGlobalScoreBg(
            candidate.globalScore
          )}`}
        >
          <h2 className="text-lg font-bold text-ink mb-3">Summary</h2>
          <p className="text-sm text-ink-2 leading-relaxed">
            The profile of {candidate.name} ({candidate.party}) receives an overall score of{' '}
            <span className={`font-bold ${getScoreColor(candidate.globalScore)}`}>
          ...
          {prevCandidate ? (
            ...
            >
              ...
              <div className="relative w-8 h-8 rounded-lg overflow-hidden">
                <Image
                  src={prevCandidate.photo}
                  alt={prevCandidate.name}
                  fill
                  className="object-cover"
                  sizes="32px"
                />
